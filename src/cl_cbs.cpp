...
#include <boost/numeric/ublas/matrix.hpp>  // Boost Basic Linear Algebra Library
...
struct State {
	...
	*/
		...
		return false;
#else
		std::vector<Segment> rectangle1{Segment(this->corner1, this->corner2), Segment(this->corner2, this->corner3), 
						Segment(this->corner3, this->corner4), Segment(this->corner4, this->corner1)};
		std::vector<Segment> rectangle2{Segment(other.corner1, other.corner2), Segment(other.corner2, other.corner3),
						Segment(other.corner3, other.corner4), Segment(other.corner4, other.corner1)};
...
struct Constraint {
	...
	double collisionScore(const State &other) const {
		...
		double temp = sqrt(pow(2 * Constants::LF, 2) + pow(Constants::carWidth, 2) - 
				pow(this->s.x - other.x, 2) + pow(this->s.y - other.y, 2));

		// std::cout << "agents distance: " << pow(this->x - other.x, 2) + pow(this->y - other.y, 2) << std::endl;
		// std::cout << "safe distance: " << pow(2 * Constants::LF, 2) + pow(Constants::carWidth, 2) << std::endl;
...
int main(int argc, char* argv[]) {
	...
	for (const auto& node : map_config["agents"]) {
		...
		startStates.emplace_back(State(start[0].as<double>(), start[1].as<double>(), start[2].as<double>()));
		// std::cout << "s: " << startStates.back() << std::endl;
		goals.emplace_back(State(goal[0].as<double>(), goal[1].as<double>(), goal[2].as<double>()));
	}
	...
	for (size_t iter = 0; iter < (double)goals.size() / batchSize; iter++) {
		...
		std::vector<State> m_starts(startStates.begin() + first, startStates.begin() + last);
	...
	if (success) {
		...
		for (const auto& s : solution) cost += s.pcost;
