...
class Environment {
 public:
  ...
  uint64_t calcIndex(const State& s) {
	return (uint64_t)(Constants::normalizeHeadingRad(s.yaw) / Constants::yawResolution) *
 ...
 private:
  ...
  std::vector<std::pair<State, double>> generatePath(State startState, int act, double deltaSteer, double deltaLength) {
...
int main() {
	...
	if (searchSuccess) {
		...
		//   std::cout << iter->first << ":" << iter->second << "->";
		// std::cout << std::endl;
		std::cout << "Solution: gscore/cost:" << solution.pcost
