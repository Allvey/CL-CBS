...
std::vector<double> dx = {r * deltat, r *sin(deltat),  r *sin(deltat), -r *deltat, -r *sin(deltat), -r *sin(deltat)};
...
class Environment {
 public:
	Environment(size_t maxx, size_t maxy, std::unordered_set<Location> obstacles,
		...
		holonomic_cost_maps = std::vector<std::vector<std::vector<double>>>(
				goals.size(), std::vector<std::vector<double>>(m_dimx, std::vector<double>(m_dimy, 0)));
	...
	bool isSolution(
		...
		std::unordered_map<State, std::tuple<State, Action, double, double>, std::hash<State>>
				cameFrom;
		cameFrom.clear();
		...
		for (auto pathidx = 0; pathidx < 5; pathidx++) {
			...
			if (generatePath(s, act, deltat, dx, next_path)) {
				for (auto iter = next_path.begin(); iter != next_path.end(); iter++) {
					...
					gscore += iter->second; // iter->second -> action cost
					if (!(next_s == path.back())) {
						cameFrom.insert(std::make_pair<>(next_s,
	...
	bool startAndGoalValid(const std::vector<State> &m_starts, const size_t iter, const int batchsize) {
		/**
		* @brief 分别判断起始点间、目标点间状态是否冲突.
		* @param m_starts 起始点状态
		* @note
		*/
 ...
 private:
	struct compare_node {
		bool operator()(const std::pair<State, double> &n1, const std::pair<State, double> &n2) const {
	...
	void updateCostmap() {
		...
		*/
		...
		boost::heap::fibonacci_heap<std::pair<State, double>, boost::heap::compare<compare_node>>
				heap;
	...
	bool generatePath(State startState, int act, double deltaSteer, double deltaLength, 
